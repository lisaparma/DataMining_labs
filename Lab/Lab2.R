...
summary(m)
names(m) # componenti del modello
# si seleziona una componente con il simbolo dollaro
m$coefficients

residui <- residuals(m) # calcolo dei residui classici

## Grafici 
par(mfrow = c(2,2)) # divisione della finestra dei plot in una tabella 2x2

hist(residui, prob = TRUE) # istogramma dei residui

plot(residui) # grafico di dispersione
